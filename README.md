...
![samples](fig_samples.png)

**模型實際預測**（上排良性、下排惡性;綠=答對、紅=答錯）

![demo](fig_demo_predictions.png)

模型對良性與明顯惡性結節判斷準確且高信心;主要失誤集中在較小、較不明顯的惡性結節（對應惡性 recall 0.70）。
...
| `make_demo.py` | 訓練並產生預測 demo 圖的腳本 |
| `fig_training.png` / `fig_confusion.png` | 訓練曲線與混淆矩陣 |
| `fig_samples.png` / `fig_demo_predictions.png` | 資料樣本圖與預測 demo 圖 |
| `results.json` | 測試集指標 |
